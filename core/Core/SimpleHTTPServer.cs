...
namespace Core
{
    public class SimpleHTTPServer
    {
        ...
        private readonly string[] indexFiles = { "index.html", "index.htm", "default.html", "default.htm" };
            ...
            if (File.Exists(filename))
            {
                try
                {
                    using (var input = new FileStream(filename, FileMode.Open))
                    {
                        ...
                        context.Response.ContentType = _mimeTypeMappings.TryGetValue(Path.GetExtension(filename), out var mime)
                            ? mime
